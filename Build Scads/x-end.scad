...
// Main block
height = 58;
translate(v=[-15,-7,height/2]) cube(size = [17,39,height], center = true);
// Bearing holder
 vertical_bearing_base();	
//Nut trap
 // Cylinder
   translate(v=[0,-19,0]) poly_cylinder(h = 8, r=10, $fn=25);
 // Hexagon
...
// Bottom pushfit rod
translate(v=[-15,-41.5,6]) rotate(a=[-90,0,0]) pushfit_rod(10,75);
// Top pushfit rod
translate(v=[-15,-41.5,rod_distance+6]) rotate(a=[-90,0,0]) pushfit_rod(10,75);

// TR Nut trap
   // Hole for the nut
    translate(v=[0,-19, 2.1]) cylinder(h = 10, d = 14.8, $fn = 6);
translate(v=[0,-19, -1]) cylinder(h = 12, d = 8.5, $fn = 25);

// Screw holes for TR nut
 //   translate(v=[0,-19, 0]) rotate([0, 0, -135]) translate([0, 9.5, -1]) cylinder(h = 10, r = 1.55, $fn=25);
//translate(v=[0,-19, 0]) rotate([0, 0, -135]) translate([0, -9.5, -1]) cylinder(h = 10, r = 1.55, $fn=25);

// Nut traps for TR nut screws
   // translate(v=[0,-19, 0]) rotate([0, 0, -135]) translate([0, 9.5, 6]) rotate([0, 0, 0])cylinder(h = 3, r = 3.3, $fn=6);

   // translate(v=[0,-19, 0]) rotate([0, 0, -135]) translate([0, -9.5, 6]) rotate([0, 0, 30])cylinder(h = 3, r = 3.2, $fn=6);
   // translate([-5.5,-19.2,6]) rotate([0,0,30]) cube([5,5,3]);
//translate([-0,-19.2,6]) rotate([0,0,60]) cube([5,10,3]);
}
